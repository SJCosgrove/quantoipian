...
cpdef calculate_exposure(positions, old_exposure):
    ...
    -------
    exposure : pd.Series
        The position exposure, ordered by the position keys.
    """
    cdef Py_ssize_t npos = len(positions)
    cdef np.ndarray[np.int64_t] index
    cdef np.ndarray[np.float64_t] values
    cdef np.float64_t exposure

    # attempt to reuse the memory of the old exposure series
    if len(old_exposure) < npos:
        index = np.empty(npos, dtype='int64')
        values = np.empty(npos, dtype='float64')
    elif len(old_exposure) > npos:
        index = old_exposure.index.values[:npos]
        values = old_exposure.values[:npos]
    else:
        index = old_exposure.index.values
        values = old_exposure.values
    ...
    for outer_position in itervalues(positions):
        ...
        exposure = inner_position.amount * inner_position.last_sale_price
        if type(inner_position.asset) is Future:
            # unchecked cast, this is safe because we do a type check above
            exposure *= (<Future> inner_position.asset).multiplier

        with cython.boundscheck(False), cython.wraparound(False):
            index[ix] = inner_position.asset.sid
            values[ix] = exposure
...
cdef class PositionStats:
    ...
    def new(cls):
        ...
        self.position_exposure_array = es.values
        return self


cdef inline np.ndarray underlying_storage(np.ndarray arr):
    cdef PyObject* base = arr.base

    if base is NULL:
        return arr
    elif isinstance(<object> base, np.ndarray):
        return underlying_storage(<object> base)
    else:
        return arr
...
cpdef calculate_position_tracker_stats(positions, PositionStats stats):
    ...
    cdef np.ndarray[np.int64_t] old_index = underlying_storage(
        stats.position_exposure_series.index.values,
    )

    cdef np.ndarray[np.float64_t] old_position_exposure = underlying_storage(
        stats.position_exposure_array,
    )
    ...
    if len(old_index) < npos:
        ...
        # array
        index = np.empty(npos, dtype='int64')
        position_exposure = np.empty(npos, dtype='float64')
        ...
        # note: we do not need to create a new series in this case
